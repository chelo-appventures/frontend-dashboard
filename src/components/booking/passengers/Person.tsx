...
export default function Person({ index }: { index: number }) {
  return (
    <>
      <SeparatorPersona title={`Pasajero ${index}`} />
      <IsResponsible />
      <SameAddress />
    </>
  );
...
function PersonalData({ index }: { index: number }) {
  const { passengerData, setPassengerData } = usePassengerData();
  const p = passengerData.passengers[index];
  return (
    <>
      <div className="flex flex-row ">
        <div className="w-1/2 mr-2">
          <LabelInput placeholder="Nombre" value={p.firstName} />
        </div>
    ...
    <>
      <div className="flex items-center">
        <input
          ...
          onChange={handlerCheckbox}
        />
        <label className="text-black p-2">Es el responsable del viaje</label>
      </div>
      {!isResponsible && <PersonalData index={0} />}
    </>
  );
}
